...
export class EncounterDrawService {
  ...
  generateEncounterPopUp(
    ...
    // }
    return (
      this.generateCommonEncounterHTML(encounter, encounterStats) +
      specifics +
      this.generateEncounterButton(encounter.id!)
    );
  }

  generateCommonEncounterHTML(
    encounter: Encounter,
    encounterStats: EncounterStatistics
  ): string {
    return `<div style="text-align: center; padding: 10px; padding-bottom: 0px; font-size: 1.15em;">
              ...
              <p style="margin-bottom: 10px;"><strong>Type:</strong> ${
                EncounterType[encounter.type]
              }</p>
              <p style="margin-bottom: 10px;"><strong>XP:</strong> ${
                encounter.xp
              }</p>
              <p style="margin-bottom: 10px;"><strong>Success Rate:</strong> ${
                encounterStats.completedCount
              } / ${
      encounterStats.completedCount + encounterStats.abandonedCount
    } completed</p>
            </div>`;
  }
  ...
  showHiddenLocationFound(): void {
    this.toastr.success(
      "You' found the hidden location, stay put for 30 seconds to complete the encounter.",
      'Hidden location found'
    );
  }

  showHiddenLocationExited(): void {
    this.toastr.warning(
      "Yo've gone too far from the hidden location.",
      'Hidden location exited'
    );
  }

  showEncounterActivated(): void {
    this.toastr.info(
      'You activated the encounter successfully.',
      'Encounter activated'
    );
  }
  ...
  showEncounterCompleted(): void {
    this.toastr.success(
      'You finished the encounter successfully.',
      'Encounter completed'
    );
  }
  ...
  showAlreadyCompleted(): void {
    this.toastr.warning(
      'You already completed the encounter..',
      'Encounter can not be activated'
    );
  }

  showEncounterAbandoned(): void {
    this.toastr.success(
      'You abandoned the active encounter.',
      'Encounter abandoned'
    );
  }
  ...
  showForcedEncounterAbandon(): void {
    this.toastr.warning(
      "Encounter abandoned, you've left the encounter range.",
      'Encounter abandoned'
    );
  }
}
