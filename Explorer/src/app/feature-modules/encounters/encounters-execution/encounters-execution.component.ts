declare global {
  interface Window {
    activateEncounter: (id: number) => void;
  }
}
...
import {
  ...
  EncounterStatistics,
  EncounterType,
} from '../model/encounters.model';
import { PagedResults } from 'src/app/shared/model/paged-results.model';
import { MapComponent } from 'src/app/shared/map/map.component';
import {
  EncounterExecution,
  EncounterExecutionStatus,
} from '../model/encounter-execution.model';
...
export class EncountersExecutionComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    window['activateEncounter'] = (id: number) =>
      this.mapComponent.activateEncounter(id);
    this.loadEncounters();
  }
  ...
  loadEncounters(): void {
    ...
    this.encounterService
      .getActiveEncounters()
      .subscribe((result: PagedResults<Encounter>) => {
        ...
        forkJoin(requests).subscribe((results: EncounterStatistics[]) => {
          let popUps = this.encounters.reduce((map, encounter, index) => {
            map.set(
              encounter.id!,
              this.encounterDrawService.generateEncounterPopUp(
                encounter,
                results[index]
              )
            );
  ...
  drawEncounters(): void {
    ...
    if (this.renderMisc) {
      encountersToDraw = encountersToDraw.concat(
        this.encounters.filter((e) => e.type.toString() === 'Misc')
      );
    }
    if (this.renderSocial) {
      encountersToDraw = encountersToDraw.concat(
        this.encounters.filter((e) => e.type.toString() === 'Social')
      );
    }
    if (this.renderHiddenLoc) {
      encountersToDraw = encountersToDraw.concat(
        this.encounters.filter((e) => e.type.toString() === 'HiddenLocation')
      );
    }
  ...
  abandonEncounter(): void {
    ...
    this.executionService.abandon(this.encounterExecution.id).subscribe({
      next: (result) => {
        ...
        if (this.encounterExecution.status.toString() === 'Abandoned') {
  ...
  activateEncounter(encounterId: any): void {
    ...
    if (!this.currentPosition) {
      this.toastr.error(
        'You need to place a pin on the map first.',
    ...
    this.executionService
      ...
      .subscribe({
        next: (result) => {
          ...
          this.mapComponent.closePopupAt(
        ...
        error: (error) => {
          if (error.error.status === 400) {
            ...
            this.encounterDrawService.showUnableToActivate(
  ...
  handleResult(result: any): void {
    if (
      !this.encounterExecution.locationEntryTimestamp &&
      result.locationEntryTimestamp
    ) {
  ...
  finishMiscEncounter(): void {
    ...
    this.executionService
      ...
      .subscribe({
        next: (result) => {
          this.encounterExecution = result;
          if (this.encounterExecution.status.toString() === 'Completed') {
            this.encounterActive = false;
            this.encounterDrawService.showEncounterCompleted();
          }
        },
